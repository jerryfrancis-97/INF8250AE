...
def add_transition(buffer: ReplayBufferStorage, transition: Transition) -> ReplayBufferStorage:
  ...
  full = cursor == max_buffer_size - 1
  cursor = (cursor + 1) % max_buffer_size
...
def sample_transition(rng: chex.PRNGKey, buffer: ReplayBufferStorage) -> Transition:
  ...
  max_elements = jnp.where(buffer.full, buffer.rewards.shape[0], buffer.cursor)
  sample_cursor = jax.random.randint(rng, (1,), 0, max_elements)
  sampled_states = jnp.squeeze(buffer.states[sample_cursor], axis=0)
  sampled_actions = jnp.squeeze(buffer.actions[sample_cursor], axis=0)
  sampled_rewards = jnp.squeeze(buffer.rewards[sample_cursor], axis=0)
  sampled_dones = jnp.squeeze(buffer.dones[sample_cursor], axis=0)
  samped_next_states = jnp.squeeze(buffer.next_states[sample_cursor], axis=0)
  ################
